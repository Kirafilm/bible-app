// download-full-bible.mjs - Download full CUV Bible from bible-api.com
import fs from 'fs';
...
async function downloadAll() {
  ...
  console.log(`Starting download: ${totalBooks} books, ~${totalChapters} chapters`);
  console.log(`Estimated time: ~${Math.ceil(totalChapters * 0.15)} minutes\n`);

  for (let i = 0; i < BOOKS.length; i++) {
    const book = BOOKS[i];
    console.log(`[${i + 1}/${totalBooks}] ${book.name} (${book.chapters} chapters)...`);
    ...
    for (let ch = 1; ch <= book.chapters; ch++) {
      ...
      if (ch % 10 === 0 || ch === book.chapters) {
        ...
        console.log(`  → ${successCount}/${book.chapters} chapters (${pct}% overall)`);
      }
    ...
    if (i % 5 === 0 || i === BOOKS.length - 1) {
      ...
      console.log(`  → Progress saved (${result.length} books complete)\n`);
    }
  }
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    for (const ch of book.chapters) {
      ...
      for (const v of ch.verses) {
        lines.push(`        { verse: ${v.verse}, text: '${v.text.replace(/'/g, "\\'")}' },`);
      }
      lines.push(`      ] },`);
    }
